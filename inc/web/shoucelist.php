...
$tid_global = $_W['tid'];
	...
	if(!(IsHasQx($tid_global,1001111,1,$schoolid)) && $toPage == 'shouceset'){
		$toPage = 'shoucelist';
	}
	if($toPage != 'shoucelist'){
...
if($operation == 'display'){
    ...
    $allxq     = pdo_fetchall("SELECT sid,sname FROM " . tablename($this->table_classify) . " where weid = :weid AND schoolid = :schoolid And type = :type ORDER BY ssort DESC", array(':weid' => $weid, ':type' => 'score', ':schoolid' => $schoolid));
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_sc) . " WHERE weid = '{$weid}' And schoolid = '{$schoolid}' $condition ORDER BY createtime LIMIT " . ($pindex - 1) * $psize . ',' . $psize);

...
function check_userlogin_all($weid,$schoolid,$openid,$userss){  //底部菜单我的冒泡_all
	if($userss != 1){
		$ite = pdo_fetch("SELECT id FROM " . tablename('wx_school_user') . " where :schoolid = schoolid And weid = :weid AND id=:id ", array(':schoolid' => $schoolid,':weid' => $weid, ':id' => $userss));
		if(!empty($ite)){
			$sessionid = $ite['id'];
		...
		$userid = pdo_fetch("SELECT id FROM " . tablename('wx_school_user') . " where :schoolid = schoolid And :weid = weid And :openid = openid  LIMIT 0,1 ", array(':weid' => $weid, ':schoolid' => $schoolid, ':openid' => $openid), 'id');
		$sessionid = $userid['id'];	
	}
	return $sessionid;
}

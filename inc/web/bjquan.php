...
$tid_global = $_W['tid'];
if (!(IsHasQx($tid_global,1001501,1,$schoolid))){
...
if($operation == 'post'){
    ...
    foreach($list as $index => $row){
        ...
        if(!empty($row['sherid'])){
            $list[$index]['picurl'] = pdo_fetchall("SELECT * FROM " . tablename($this->table_media) . " WHERE weid = :weid AND schoolid = :schoolid AND sherid =:sherid  ORDER BY id ASC", array(':weid' => $weid,':schoolid' => $schoolid,':sherid' => $row['sherid']));
	...
	$bjq = pdo_fetch("SELECT ali_vod_id FROM " . tablename($this->table_bjq) . " where id = :id", array(':id' => $id));
	if($bjq['ali_vod_id']){
		mload()->model('ali');

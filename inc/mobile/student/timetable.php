 ...
 */
        ...
        $school = pdo_fetch("SELECT title,style2,is_kb,logo,headcolor FROM " . tablename($this->table_index) . " where weid = :weid AND id=:id ORDER BY ssort DESC", array(':weid' => $weid, ':id' => $schoolid));
		...
		if($it){
			...
			$cook = pdo_fetch("SELECT * FROM " . tablename($this->table_timetable) . " WHERE schoolid = {$schoolid} And bj_id = {$student['bj_id']} And ishow = 1 $condition" );
			$week = date("w",$endtime);
			if($week ==1){
				if($cook['monday']){

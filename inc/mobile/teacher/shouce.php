 ...
 */
		...
		$school = pdo_fetch("SELECT style3,title,spic,tpic,headcolor FROM " . tablename($this->table_index) . " where weid = :weid AND id=:id", array(':weid' => $weid, ':id' => $schoolid));
		$student = pdo_fetch("SELECT s_name,icon,bj_id FROM " . tablename($this->table_students) . " where id = :id", array(':id' => $sid));
		$liststu = pdo_fetchall("SELECT id FROM " . tablename($this->table_students) . " where :schoolid = schoolid And :weid = weid And :bj_id = bj_id ORDER BY id DESC", array(
			 ':weid' => $weid,
			 ':schoolid' => $schoolid,
			 ':bj_id' => $student['bj_id']
			 ));
		if($liststu){	
			$listss = array();
			foreach($liststu as $key){
				if(!in_array($listss,array($key['id']))){
					$isdp = pdo_fetch("SELECT id FROM " . tablename ($this->table_scforxs) . " where schoolid = :schoolid And scid = :scid And sid = :sid And type = :type", array(':schoolid' => $schoolid, ':scid' => $scid, ':type' => 2, ':sid' => $key['id']));
					if(!$isdp){
						$listss[] = $key['id'];
					}
				}
		...
		// print_r($listss);
		$offset = array_search($sid,$listss);
		$nextid = $listss["$offset"+1];
		$lastid = $listss[count($listss)-1];
        if(!empty($userid['id'])){
			if($_GPC['op'] =='savedata'){
			...
			if($_GPC['op'] =='edite'){
				...
				if($_GPC['type'] =='home'){
					$allpl = pdo_fetchall("SELECT jzword,tid,createtime,userid FROM " . tablename($this->table_scforxs) . " where sid = :sid And scid = :scid And type = :type And fromto = :fromto ORDER BY createtime DESC", array(
						':sid' => $sid,
						':scid' => $scid,
						':type' => 1,
						':fromto' => 2

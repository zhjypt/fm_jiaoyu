...
$tid_global = $_W['tid'];
if (!(IsHasQx($tid_global,1002301,1,$schoolid))){
...
if($operation == 'display'){
    ...
    if (is_showpf()) {
        if ($_GPC['am_pm'] == 1) {
            $condition .= " AND FROM_UNIXTIME(createtime,'%p') = 'AM' ";
        } elseif ($_GPC['am_pm'] == 2) {
            $condition .= " AND FROM_UNIXTIME(createtime,'%p') = 'PM' ";
        }
    }

	
	
    if(!empty($_GPC['createtime']['start']) || !empty($_GPC['createtime']['start'])){
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_checklog) . " WHERE weid = '{$weid}'and sc_ap = 0 AND schoolid = '{$schoolid}' $condition ORDER BY createtime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
	$checkpic = pdo_fetchall("SELECT id FROM " . tablename($this->table_checklog) . " WHERE weid = '{$weid}' AND schoolid = '{$schoolid}' and sc_ap = 0 AND (pic LIKE '%wmpickq%' Or pic2 LIKE '%wmpickq%')");
    ...
    foreach($list as $index => $row){
        ...
        $idcard                  = pdo_fetch("SELECT pname,cardtype FROM " . tablename($this->table_idcard) . " WHERE idcard = '{$row['cardid']}' And schoolid = '{$schoolid}' ");
        ...
        $list[$index]['cardtype']= $idcard['cardtype'];
    ...
    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->table_checklog) . " WHERE weid = '{$weid}' AND schoolid ={$schoolid} and sc_ap = 0 $condition");

    $pager = pagination($total, $pindex, $psize);
    //////////导出数据/////////////////
    if($_GPC['out_put'] == 'output'){
		$school = pdo_fetch("SELECT lat,lng FROM " . tablename($this->table_index) . " WHERE weid = '{$weid}' AND id ={$schoolid} ");
        $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_checklog) . " WHERE weid = '{$weid}' AND schoolid ={$schoolid} and sc_ap = 0 $condition ORDER BY id DESC");
        $ii   = 0;
        foreach($list as $index => $row){
            $student             = pdo_fetch("SELECT s_name FROM " . tablename($this->table_students) . " WHERE id = '{$row['sid']}' ");
            $teacher             = pdo_fetch("SELECT tname FROM " . tablename($this->table_teachers) . " WHERE id = '{$row['tid']}' ");
            ...
            if(is_showpf()){
                if($row['lon'] != 0 && $row['lat'] != 0 ){
                    $gg = bd_encrypt($row['lon'],$row['lat']);
                    $rand_this = getDistance($gg['bd_lat'],$gg['bd_lon'],$school['lat'],$school['lng']);
                    $arr[$ii]['rand_this'] = intval($rand_this);
                }else{
                    $arr[$ii]['rand_this'] = '未记录定位信息';
                }
            }
            $ii++;
        }

        $excel_title = array('考勤机', '刷卡人', '身份', '卡号', '班级', '进出状态', '持卡人姓名', '刷卡时间', '体温', '是否拍照');
        if(is_showpf()){
            $excel_title = array('考勤机', '刷卡人', '身份', '卡号', '班级', '进出状态', '持卡人姓名', '刷卡时间', '体温', '是否拍照','打卡距离/米');
        }
        $this->exportexcel($arr,  $excel_title , '考勤记录');
        exit();
    }
    ////////////////////////////////
	if($_GPC['download'] == 'download'){
		$listss = pdo_fetchall("SELECT id,pic,pic2 FROM " . tablename($this->table_checklog) . " WHERE weid = '{$weid}' AND schoolid ='{$schoolid}' and sc_ap = 0  ORDER BY createtime DESC");
		$rowcount    = 0;
		$notrowcount = 0;		

...
$operation = in_array($_GPC ['op'], array('default', 'login', 'classinfo', 'check', 'shexiangtou', 'banner', 'video', 'start', 'info','info_report','online','report','getinfo')) ? $_GPC ['op'] : 'default';
$weid      = trim($_GPC['i']);
...
if($operation == 'classinfo'){
    ...
    if(!empty($ckmac)){
        ...
        $users = pdo_fetchall("SELECT idcard as card_id, sid as baby_id, bj_id as CLASS_ID, usertype as USERTYPE, sid as SID, tid as TID, pard as PARD ,cardtype FROM " . tablename($this->table_idcard) . " WHERE weid = '{$weid}' And schoolid = '{$school['id']}' And is_on = 1 And severend > '{$times}' $near_time ORDER BY createtime DESC");
        if($users){
	...
	if($checkorder){
		if(!empty($checkorder['dotime'])){
			if($checkorder['dotime'] < $nowtimes){
				...
				pdo_update($this->table_online, array('isread' => 1), array('id' => $checkorder['id']));
			}else{
				$result['data'] = array(
					array(
						'server_time' => $nowtime,
...
if ($operation == 'getinfo') {
	...
	if (!is_error($result)) {
		$showurl = $this->createImageUrlCenterForUser("https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=" . $result['ticket'], 9999999999, 0, $schoolid);
		$urlarr = explode('/',$showurl);
		$qrurls = "images/fm_jiaoyu/".$urlarr['4'];	
		if (!empty($_W['setting']['remote']['type'])) {
			$remotestatus = file_remote_upload($qrurls);
		}
		$qrimg = tomedia($qrurls);			
	}else{
		$qrimg = '';			
	}    
	$result = getbasicdata($weid,$schoolid,$_W['uniaccount']['name'],$_W['siteroot']);
	$result['qrimg'] = $qrimg;
	echo json_encode($result);
}
?>
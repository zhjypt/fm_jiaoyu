...
var ALI_URL = "http://static.weixiaotong.com.cn/";
var ALI_URL_VIEDO = "http://static.weixiaotong.com.cn/";
	//"http://weixt-static.oss-cn-qingdao.aliyuncs.com/";
...
function wxImageShowByClass(node){
	var srcList = new Array();
	var imgs = $(node).closest('.parent').find("img.team");
	var curSrc = $(node).find("img")[0]['src'].split("@");
	var curImgIndex=0;
	for(i=0;i<imgs.length;i++){
		var imgsrc = imgs[i]['src'].split("@");
...
function wxImageShowWeUI(node){
	var srcList = new Array();
	var imgs = $(node).closest("ul").find("li");
	var curSrc = $(node).attr("url").split("@");
	var curImgIndex=0;
	for(i=0;i<imgs.length;i++){
		var imgsrc = $(imgs[i]).attr("url").split("@")
		if(imgsrc.length>1){
			srcList.push(imgsrc[0].replace(ALI_URL,ALI_URL_VIEDO));
		}else{
			srcList.push(imgsrc[0]);
...
function wxImageShowWithArray(node){
	var srcList = new Array();
	var imgs = $(node).attr("images").split(",");
	var curImgIndex=0;
	for(i=0;i<imgs.length;i++){

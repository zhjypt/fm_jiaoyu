 ...
 */
        ...
        if($it){
			...
			$weekplanlist = pdo_fetchall("SELECT * FROM " . tablename($this->table_zjh) . " where schoolid = '{$schoolid}' And weid = '{$weid}' And bj_id = '{$student['bj_id']}' And is_on = 2 ORDER BY createtime,is_on,end DESC");

...
if($operation == 'display'){
    ...
    $list  = pdo_fetchall("SELECT * FROM " . tablename($this->table_news) . " WHERE weid = '{$weid}' And schoolid = '{$schoolid}' And type = 'wenzhang' $condition ORDER BY displayorder DESC, id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->table_news) . " WHERE weid = '{$weid}' And schoolid = '{$schoolid}' And type = 'wenzhang'");
